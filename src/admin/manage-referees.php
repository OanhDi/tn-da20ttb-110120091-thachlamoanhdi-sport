...
if (strlen($_SESSION['alogin']) == 0) {
	...
	<head>
		...
		<title>TVU Sport Center |Admin Manage Referee </title>
				...
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<h2 class="page-title">Giao Nhiệm Vụ Trọng Tài</h2>

							<!-- Zero Configuration Table -->
							<div class="panel panel-default">
								<div class="panel-heading">Chi Tiết Nhiệm Vụ</div>
									...
									<div class="row mb-4">
										<div class="col-md-12">
											...
											<div class="btn-group col-md-8" style="padding-bottom: 1rem">
												...
												<div class="col-md-4">
													<button id="anotherButton" class="btn btn-primary">Chọn</button>
												</div>
											</div>
										</div>
									</div>
				...
				async function fetchData(Bkdate, FieldType, Field) {
					try {
						const searchMatchResponse = await $.ajax({
							url: 'match/getmatch-schedule.php',
							method: 'POST',
							data: {
								BookingDate: Bkdate,
						...
						if (results && results.length > 0) {
							results.forEach((result, index) => {
								...
								const add = `	<tr>								 
															...
															<td><a href="#" class="manage-referee"><i
																			class="fa fa-check"></i></a>&nbsp;&nbsp;
																<a href="#" class="delete-referee"><i
							...
							$('.edit-link').on('click', function (e) {
								...
								if (checkbox.prop('checked')) {  // Use prop('checked') instead of is(':checked')
									row.find('.trongtai').text('text');
									window.location.href = `edit-field.php?id=${matchID}`;
								} else {
									alert('Please check the checkbox first.');
								}
							});
							$('.delete-referee').on('click', function (e) {
								e.preventDefault();
								const row = $(this).closest('tr');
								row.find('.trongtai').empty();  // Xóa nội dung của thẻ <td class="trongtai">
							});
						}
					} catch (e) {
