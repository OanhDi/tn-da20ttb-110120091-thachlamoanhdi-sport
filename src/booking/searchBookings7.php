...
WHERE bk.BookingDate = :bookingDate AND ft.FieldTypeID = '1' AND (f.FieldGroup IS NOT NULL OR f.FieldGroup <> '')";
  ...
  foreach ($fieldIdTmList as $item) {
    $conditions[] = "(f.FieldGroup IS NULL AND tm.idtm = " . $item['idtm'] . ")";
  }
  ...
  $sql = "WITH a AS (
      ...
      AND bk.BookingDate = :bookingDate AND (f.FieldGroup IS NULL OR f.FieldGroup = '')
    ...
    AND tm.StartTime BETWEEN :startTime AND :endTime AND (f.FieldGroup IS NULL OR f.FieldGroup = '')
