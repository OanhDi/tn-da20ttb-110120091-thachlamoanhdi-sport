...
try {
    ...
    //Get info
    $sql_get_booking = "SELECT BookingID, CustomerID, AwayTeam, idfm, BookingDate FROM tblbookings bk WHERE idfm = :idfm AND BookingDate = :bookingdate AND Status = '2';";
    $query_get_booking = $dbh->prepare($sql_get_booking);
    $query_get_booking->bindParam(':idfm', $idfm);
    $query_get_booking->bindParam(':bookingdate', $date_bk);
    ...
    $sql_update_booking = "UPDATE tblbookings SET Status = '1',
        AwayTeam = NULL
        WHERE idfm = :idfm AND BookingDate = :bookingdate AND Status = '2';";
